...
use crate::route::{catalog, consumer, db, functions, stream};
use axum::{
    Router,
    routing::{get, post},
};
...
pub fn build_router(
    ...
    Router::new()
        ...
        .nest(prefix_values, api_values())
        .nest(prefix_directory, api_db())
        .route(prefix_ws, get(ws_handler))
        .with_state(state)
}
...
fn setting_routes() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/plugin/resolve/{setting_code}",
            get(get_plugin_resolve_cache),
        )
        .route("/module/cache/{setting_code}", get(get_module_cache))
        .route(
            "/module/resolve/{setting_code}",
            get(get_module_resolve_cache),
        )
}
