...
pub struct GetStreamSetting {
    pub stream: Stream,
}

#[async_trait]
impl HostFn for GetStreamSetting {
    ...
    async fn call(&self, input: &[u8]) -> Result<Option<Vec<u8>>, RuntimeError> {
        ...
        let function = self.stream.clone();
        let settings = tokio::task::spawn_blocking(move || function.get_stream_setting(&code))
            .await
