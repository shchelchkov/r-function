...
impl Processor {
    ...
    pub async fn handle_batch(&self, msgs: Vec<Message>) -> Vec<Result<(), ()>> {
        ...
        for (setting_code, group) in groups {
            let result = match self.process_group(&msgs, &setting_code, &group).await {
                Ok(result) => result,
                Err(e) => {
    ...
    async fn process_group(
        ...
        self.producer
            .send_objects(setting_code, group.out_key.as_deref(), payload).await
            .map_err(|e| {
    ...
    async fn route_group_dlq(&self, msgs: &[Message], group: &Group, reason: &str) -> Result<(), ProcessError> {
